...
import { DownloadIcon, FilesIcon, HomeIcon, LayoutDashboard, UserCircle } from "lucide-react";
...
const NavBar = () => {
  ...
  return (
    <nav
      ...
      <ul
        className={`grid h-full max-w-2xl mx-auto font-medium divide-x divide-gray-400 ${userData.role.name === "SUPER_ADMIN" ? "grid-cols-5" : "grid-cols-4"}`}>
        {navbarMenus.map((menu) => (
          <>
            ...
            {menu.title === "Dashboard" && userData.role.name === "SUPER_ADMIN" && (
              <NavLink
                to={menu.path}
                key={menu.title}
                className={({ isActive }) =>

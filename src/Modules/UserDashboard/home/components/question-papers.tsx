...
import { useGetQuestionPaperList } from "@/hooks/useQuestionPaper";

const QuestionPapers = () => {
  // const { isLoading: loading, isSuccess, data: questionPapers, isError, error } = useGetQuestionPaper({});
  const { data: questionPaperList, isLoading: questionPaperListLoading, isError, error } = useGetQuestionPaperList({});

  if (isError) return <>{errorHandler(error)}!</>;
  if (questionPaperListLoading) return <>Loading...</>;
  ...
  return (
    <Card className="p-2 rounded-md shadow-lg bg-white/20">
      {questionPaperList && <QuestionPaperList questionPapers={questionPaperList} />}
    </Card>
  );
};
